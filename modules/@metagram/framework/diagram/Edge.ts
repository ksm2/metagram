...
export abstract class Edge<M extends ModelElement> extends DiagramElement<M> {
   ...
   */
  get bounds(): Bounds {
    let minX = Math.min(this._source.absoluteLocation.x, this._target.absoluteLocation.x);
    let minY = Math.min(this._source.absoluteLocation.y, this._target.absoluteLocation.y);
